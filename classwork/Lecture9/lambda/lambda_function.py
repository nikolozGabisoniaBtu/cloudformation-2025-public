...
API_TOKEN = "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"

headers = {"Authorization": f"Bearer {API_TOKEN}"}
API_URL = "https://router.huggingface.co/hf-inference/models/facebook/detr-resnet-50"
DYNAMODB_TABLE = "lecture9-table"
s3_client = boto3.client("s3")


def query_image(f):
    data = f.read()
    http_request = Request(API_URL, data=data, headers=headers)
    http_request.add_header('Content-Type', 'image/jpeg')
